...
class _PlaygroundScreenState extends State<PlaygroundScreen> {
  ...
  final _systemController = TextEditingController(
      text: 'Tu es Mistral, un assistant IA développé par Mistral AI. Tu réponds de façon claire, concise et utile.');
  ...
  Future<void> _send() async {
    ...
    if (_apiKey.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Clé API manquante — configurez-la dans Chat Mistral'),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final msg = err['message'] ?? 'Erreur ${response.statusCode}';
        setState(() {
      ...
      children: [
        ...
        Wrap(
          ...
          children: _playgroundModels
              .map((m) => ChoiceChip(
                    label: Text(m, style: const TextStyle(fontSize: 11, fontFamily: 'monospace')),
                    ...
                    labelStyle: TextStyle(
                        color: _selectedModel == m
                            ? context.accentLight
                            : context.isLight ? const Color(0xFF5D3A1A) : const Color(0xFF9E9E9E)),
                    side: BorderSide(
                        color: _selectedModel == m
                            ? context.accentLight
                            : context.codeBorder),
        ...
        _sliderRow('Temperature', _temperature, _temperature.toStringAsFixed(2),
            0.0, 2.0, 20, (v) => setState(() => _temperature = v), context),
        _sliderRow('Top P', _topP, _topP.toStringAsFixed(2),
            0.0, 1.0, 20, (v) => setState(() => _topP = v), context),
        _sliderRow('Max Tokens', _maxTokens.toDouble(), '$_maxTokens',
            128, 8192, 32, (v) => setState(() => _maxTokens = v.round()), context),
        const SizedBox(height: 16),
        ...
        GestureDetector(
          ...
          child: Row(
            children: [
              Text('Prompt système',
                  style: TextStyle(color: context.accentLight, fontWeight: FontWeight.w600, fontSize: 13)),
        ...
        if (_showSystemPrompt) ...[
          ...
          TextField(
            ...
            maxLines: 4,
            style: TextStyle(color: context.isLight ? const Color(0xFF3E2000) : Colors.white,
        ...
        TextField(
          ...
          minLines: 3,
          style: TextStyle(
              color: context.isLight ? const Color(0xFF3E2000) : Colors.white, fontSize: 14),
          decoration: InputDecoration(
  ...
  Widget build(BuildContext context) {
    final items = [
      (Icons.tune, 'Ajustez les paramètres',
          'Temperature contrôle la créativité (0 = déterministe, 1 = très créatif). Top P filtre le vocabulaire utilisé. Max Tokens limite la longueur de la réponse.'),
      (Icons.model_training, 'Choisissez le bon modèle',
          'mistral-large pour les tâches complexes, mistral-small pour un bon équilibre vitesse/qualité, codestral pour le code, open-mistral-7b pour la rapidité maximale.'),
      (Icons.bolt, 'Pourquoi ce playground ?',
          'Testez en direct l\'impact de chaque paramètre sur les réponses de l\'API Mistral, sans écrire de code. Idéal pour calibrer vos prompts avant intégration.'),
    ];
